# ============================================================================
# AUDITORÍA DE CALIDAD DE HISTORIAS CLÍNICAS - APLICACIÓN SHINY
# VERSIÓN CON CARGA AUTOMÁTICA DESDE CARPETA "data/"
# ============================================================================
...
truncar_texto <- function(texto, max_caracteres = 40) {
  ...
  resultado <- ifelse(nchar(texto) > max_caracteres,
...
archivos_esperados <- list(
  "ANAMNESIS_FILIACION" = "ANAMNESIS FILIACION.csv",
  "ENFERMEDAD_ACTUAL" = "ENFERMEDAD ACTUAL Y ANTECEDENTES.csv",
  "EXAMEN_CLINICO" = "EXAMEN CLINICO GENERAL.csv",
  "DIAGNOSTICOS" = "DIAGNOSTICOS.csv",
  "PLAN_TRABAJO" = "PLAN DE TRABAJO.csv",
  "TRATAMIENTO" = "TRATAMIENTO.csv",
  "NOTAS_EVOLUCION" = "NOTAS DE EVOLUCION.csv",
  "REGISTROS_ENFERMERIA" = "REGISTROS DE ENFERMERIA OBSTETRICIA.csv",
  "ATRIBUTOS_HC" = "ATRIBUTOS DE LA HISTORIA CLINICA.csv",
  "FORMATOS_ESPECIALES" = "FORMATOS ESPECIALES.csv",
  "INDICACIONES_ALTA" = "ESPECIFICA INDICACIONES DE ALTA.csv"
)

# Procesar archivo individual
procesar_archivo <- function(ruta, seccion) {
  ...
  if(!"ITEM" %in% names(df)) return(NULL)
  ...
  df %>%
    ...
    filter(!is.na(CALIFICACION_TXT)) %>%
    select(ITEM, SECCION, VARIABLE, CALIFICACION, CALIFICACION_TXT)
}

# Cargar todos los archivos automáticamente
cargar_datos_automatico <- function() {
  
  carpeta_datos <- "data"
  
  if(!dir.exists(carpeta_datos)) {
    dir.create(carpeta_datos, recursive = TRUE)
    message("📁 Carpeta 'data/' creada. Coloque los archivos CSV allí.")
    return(NULL)
  }
  ...
  for(seccion in names(archivos_esperados)) {
    nombre_archivo <- archivos_esperados[[seccion]]
    ruta <- file.path(carpeta_datos, nombre_archivo)
    
    if(file.exists(ruta)) {
      resultado <- procesar_archivo(ruta, seccion)
      if(!is.null(resultado) && nrow(resultado) > 0) {
        lista_datos[[seccion]] <- resultado
        archivos_encontrados <- archivos_encontrados + 1
      }
    } else {
      archivos_faltantes <- c(archivos_faltantes, nombre_archivo)
    }
  }
  
  if(archivos_encontrados == 0) {
    message("⚠️ No se encontraron archivos CSV en la carpeta 'data/'")
  ...
  message(paste0("✅ ", archivos_encontrados, " de ", length(archivos_esperados), " archivos cargados"))
...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    ...
    box(
      ...
      collapsible = TRUE,
      collapsed = TRUE,
      HTML("Datos cargados automáticamente desde carpeta 'data/'")
    )
  ),
  ...
  dashboardBody(
    tags$head(
      tags$style(HTML("
        ...
        .box-body { flex: 1; overflow: auto; padding: 10px; }
        .box-title { padding: 10px 15px; font-size: 14px; font-weight: bold; }
        .plotly { height: 100% !important; }
        .dataTables_wrapper { font-size: 11px; padding: 5px; }
        .dataTables_wrapper .dataTables_scrollBody { max-height: 450px !important; }
        table.dataTable { font-size: 11px; width: 100% !important; }
        table.dataTable th, table.dataTable td { padding: 6px 10px; white-space: nowrap; }
        .info-box { min-height: 80px; margin-bottom: 15px; }
        .btn { margin: 5px; }
        .fluid-row { margin-left: 0; margin-right: 0; }
        .col-sm-7, .col-sm-5, .col-sm-6, .col-sm-4 { padding-left: 10px; padding-right: 10px; }
      "))
    ),
    ...
    tabItems(
      tabItem(tabName = "dashboard",
              ...
              fluidRow(
                box(width = 12, title = "📊 Desempeño por Sección", status = "primary", solidHeader = TRUE,
      ...
      tabItem(tabName = "secciones",
              fluidRow(
                box(width = 6, title = "📈 Perfil de Calidad", status = "info", solidHeader = TRUE,
              ...
              fluidRow(
                box(width = 12, title = "🗺️ Treemap", status = "info", solidHeader = TRUE,
      ...
      tabItem(tabName = "graficos_seccion",
              fluidRow(
                ...
                box(width = 8, title = "📊 Resumen", status = "info", solidHeader = TRUE,
                    uiOutput("resumen_seccion_ui"))
              ),
              fluidRow(
                box(width = 7, title = "📊 Distribución por Ítem", status = "success", solidHeader = TRUE,
                    plotlyOutput("plot_barras_seccion_detalle", height = "650px")),
                box(width = 5, title = "📋 Tabulación", status = "warning", solidHeader = TRUE,
              ...
              fluidRow(
                box(width = 6, title = "📈 Completitud", status = "warning", solidHeader = TRUE,
                    plotlyOutput("plot_completitud_items", height = "550px")),
                box(width = 6, title = "🔴 Problemas", status = "danger", solidHeader = TRUE,
      ...
      tabItem(tabName = "criticos",
              fluidRow(
                box(width = 8, title = "⚠️ Top 20 Items", status = "danger", solidHeader = TRUE,
                    plotlyOutput("plot_top_criticos", height = "650px")),
                box(width = 4, title = "📋 Historias Riesgo", status = "danger", solidHeader = TRUE,
                    DT::dataTableOutput("tabla_historias_fallando"))
              ),
              fluidRow(
                box(width = 12, title = "🔗 Correlación", status = "warning", solidHeader = TRUE,
      ...
      tabItem(tabName = "exportar",
              fluidRow(
                box(width = 12, title = "💾 Descargar", status = "success", solidHeader = TRUE,
                    p("Descargue el Excel con todos los datos."),
                    br(),
                    downloadButton("download_excel", "📥 Excel Completo", class = "btn-lg btn-success", icon = icon("file-excel")),
                    br(), br(),
                    downloadButton("download_graficos_seccion", "📥 Gráficos PDF", class = "btn-lg btn-primary", icon = icon("file-pdf")))
              )
      )
    )
  )
...
server <- function(input, output, session) {
  ...
  datos_iniciales <- reactiveVal(NULL)
  
  observe({
    showNotification("🔄 Cargando datos automáticamente...", type = "message", duration = 3)
    datos_crudos <- cargar_datos_automatico()
    
    if(!is.null(datos_crudos)) {
      datos_procesados <- procesar_datos_completos(datos_crudos)
      datos_iniciales(datos_procesados)
      showNotification(paste0("✅ ", datos_procesados$resumen_general$TOTAL_HISTORIAS, 
                              " historias cargadas"), type = "message", duration = 5)
    } else {
      showNotification("⚠️ No se encontraron datos en carpeta 'data/'", type = "warning", duration = 10)
    }
  })
  ...
  output$plot_donut <- renderPlotly({
    ...
    fig %>% layout(title = "Distribución", showlegend = TRUE, margin = list(t = 40, b = 40))
  })
  ...
  output$plot_gauge <- renderPlotly({
    ...
    fig <- plot_ly() %>%
      add_trace(type = 'indicator', mode = "gauge+number", value = valor,
                gauge = list(axis = list(range = list(NULL, 100)),
                             steps = list(list(range = c(0, 50), color = "#e74c3c"),
  ...
  output$plot_hist <- renderPlotly({
    ...
    fig %>% layout(title = "Distribución", margin = list(t = 40, b = 40))
  })
  ...
  output$plot_barras_seccion <- renderPlotly({
    ...
    fig <- plot_ly(d$resumen_por_seccion, x = ~PORCENTAJE_SECCION, y = ~SECCION, type = 'bar', orientation = 'h',
                   marker = list(color = ~PORCENTAJE_SECCION, colorscale = 'RdYlGn'))
    fig %>% layout(title = "Desempeño", margin = list(l = 150, t = 40, b = 40))
  })
  ...
  output$plot_perfil_lineas <- renderPlotly({
    ...
    fig <- plot_ly(d$resumen_por_seccion, x = ~SECCION_SHORT, y = ~PORCENTAJE_SECCION, 
                   type = 'scatter', mode = 'lines+markers', line = list(color = '#3498db', width = 3))
    fig %>% layout(title = "Perfil", yaxis = list(range = c(0, 100)), margin = list(t = 40, b = 80))
  })
  ...
  output$plot_heatmap <- renderPlotly({
    ...
    fig %>% layout(title = "Calificaciones", margin = list(l = 150, t = 40, b = 40))
  })
  ...
  output$plot_violin <- renderPlotly({
    ...
    fig <- plot_ly(score_seccion_hc, y = ~PCT_SECCION, x = ~SECCION, type = 'violin', box = list(visible = TRUE))
    fig %>% layout(title = "Distribución", margin = list(t = 40, b = 80))
  })
  ...
  output$plot_treemap <- renderPlotly({
    ...
    treemap_data$Calidad <- cut(treemap_data$PORCENTAJE_SECCION, breaks = c(0, 50, 70, 90, 100), 
                                labels = c("Inadecuada", "Deficiente", "Aceptable", "Excelente"))
    fig <- plot_ly(treemap_data, labels = ~SECCION, parents = ~"", values = ~PESO_MAXIMO, color = ~Calidad, type = 'treemap')
    fig %>% layout(title = "Peso vs Calidad", margin = list(t = 40, b = 20))
  })
  ...
  output$resumen_seccion_ui <- renderUI({
    ...
    if(nrow(resumen_seccion) == 0) return(p("Sin datos"))
    tagList(
      fluidRow(
        column(4, infoBox("Obtenido", round(resumen_seccion$PUNTAJE_OBTENIDO_SECCION, 2), icon = icon("check"), color = "green", fill = TRUE)),
        column(4, infoBox("Máximo", round(resumen_seccion$PUNTAJE_MAXIMO_SECCION, 2), icon = icon("star"), color = "blue", fill = TRUE)),
        column(4, infoBox("%", paste0(resumen_seccion$PORCENTAJE_SECCION, "%"), icon = icon("chart-line"), 
                          color = ifelse(resumen_seccion$PORCENTAJE_SECCION >= 80, "green", "red"), fill = TRUE))
      )
    )
  })
  ...
  output$plot_barras_seccion_detalle <- renderPlotly({
    ...
    items_unicos <- d$datos_con_pesos %>% filter(SECCION == seccion_codigo) %>% distinct(VARIABLE) %>% pull(VARIABLE)
    if(length(items_unicos) == 0) return(plot_ly() %>% layout(title = "Sin datos", height = 300))
    
    tratamiento_detalle <- d$datos_con_pesos %>%
      filter(SECCION == seccion_codigo) %>%
      group_by(VARIABLE, CALIFICACION_TXT) %>% summarise(Cantidad = n(), .groups = "drop") %>%
      tidyr::complete(VARIABLE = items_unicos, CALIFICACION_TXT = c("COMPLETO", "INCOMPLETO", "NO EXISTE", "NO APLICA"), fill = list(Cantidad = 0)) %>%
      mutate(ITEM_NUM = match(VARIABLE, items_unicos),
             VARIABLE_LIMPIA = iconv(as.character(VARIABLE), from = "UTF-8", to = "UTF-8", sub = ""),
             VARIABLE_CORTA = paste0(ITEM_NUM, ". ", truncar_texto(VARIABLE_LIMPIA, max_car)),
             VARIABLE_COMPLETA = VARIABLE_LIMPIA,
             CALIFICACION_TXT = factor(CALIFICACION_TXT, levels = c("COMPLETO", "INCOMPLETO", "NO EXISTE", "NO APLICA"))) %>%
      arrange(ITEM_NUM, CALIFICACION_TXT)
    
    fig <- plot_ly(tratamiento_detalle, x = ~VARIABLE_CORTA, y = ~Cantidad, color = ~CALIFICACION_TXT, type = 'bar',
                   colors = c("#2ecc71", "#f39c12", "#e74c3c", "#95a5a6"), hoverinfo = "text",
                   hovertext = ~paste0("<b>", VARIABLE_COMPLETA, "</b><br>", CALIFICACION_TXT, ": ", Cantidad)) %>%
      layout(title = paste("Distribución -", input$seccion_seleccionada), xaxis = list(tickangle = -45, tickfont = list(size = 8)),
             yaxis = list(title = "Cantidad"), barmode = 'stack', legend = list(orientation = 'h', y = -0.15),
             margin = list(l = 50, r = 50, t = 60, b = 150), height = 600)
    fig
  })
  ...
  output$tabla_distribucion_items <- DT::renderDataTable({
    ...
    tryCatch({
      datos_seccion <- d$datos_con_pesos %>% filter(SECCION == seccion_codigo)
      todas_categorias <- expand.grid(VARIABLE = unique(datos_seccion$VARIABLE),
                                      CALIFICACION_TXT = c("COMPLETO", "INCOMPLETO", "NO EXISTE", "NO APLICA"), stringsAsFactors = FALSE)
      conteo_real <- datos_seccion %>% group_by(VARIABLE, CALIFICACION_TXT) %>% summarise(Cantidad = n(), .groups = "drop")
      tabla_completa <- todas_categorias %>% left_join(conteo_real, by = c("VARIABLE", "CALIFICACION_TXT")) %>% mutate(Cantidad = ifelse(is.na(Cantidad), 0, Cantidad))
      tabla_pivot <- tabla_completa %>% tidyr::pivot_wider(names_from = CALIFICACION_TXT, values_from = Cantidad, values_fill = 0)
      
      if(!"COMPLETO" %in% names(tabla_pivot)) tabla_pivot$COMPLETO <- 0
      if(!"INCOMPLETO" %in% names(tabla_pivot)) tabla_pivot$INCOMPLETO <- 0
      if(!"NO EXISTE" %in% names(tabla_pivot)) tabla_pivot$"NO EXISTE" <- 0
      if(!"NO APLICA" %in% names(tabla_pivot)) tabla_pivot$"NO APLICA" <- 0
      
      tabla_datos <- tabla_pivot %>% mutate(Total = COMPLETO + INCOMPLETO + `NO EXISTE` + `NO APLICA`,
                                            Porcentaje_Completitud = ifelse(Total > 0, round((COMPLETO / Total) * 100, 1), 0),
                                            ITEM = row_number()) %>%
        select(ITEM, VARIABLE, COMPLETO, INCOMPLETO, `NO EXISTE`, `NO APLICA`, Total, Porcentaje_Completitud) %>% arrange(ITEM)
      
      if(nrow(tabla_datos) == 0) return(data.frame(Mensaje = "Sin datos"))
      
      DT::datatable(tabla_datos, options = list(pageLength = 15, scrollX = TRUE, scrollY = "550px", autoWidth = TRUE),
                    rownames = FALSE, class = 'table table-striped table-bordered table-condensed', filter = 'top', escape = FALSE)
    }, error = function(e) {
  ...
  output$download_tabla_distribucion <- downloadHandler(
    filename = function() { paste0("Tabla_", gsub("[^a-zA-Z0-9]", "_", input$seccion_seleccionada), "_", Sys.Date(), ".csv") },
  ...
  output$plot_completitud_items <- renderPlotly({
    ...
    completitud_items <- d$datos_con_pesos %>%
      ...
      group_by(VARIABLE) %>% summarise(Total = n(), Completos = sum(CALIFICACION == 1),
                                       Porcentaje = round((Completos / Total) * 100, 2), .groups = "drop") %>%
    ...
    fig <- plot_ly(completitud_items, x = ~Porcentaje, y = ~VARIABLE_CORTA, type = 'bar', orientation = 'h',
                   marker = list(color = ~Porcentaje, colorscale = list(c(0, "#e74c3c"), c(0.5, "#f39c12"), c(1, "#2ecc71")), showscale = TRUE),
                   hoverinfo = "text", hovertext = ~paste0("<b>", VARIABLE_COMPLETA, "</b><br>", Porcentaje, "%")) %>%
      layout(title = "Completitud", xaxis = list(title = "%", range = c(0, 100)), yaxis = list(title = "Ítems", tickfont = list(size = 9)),
             margin = list(l = 300, r = 50, t = 40, b = 50), height = 500)
    fig
  })
  ...
  output$plot_items_problemas <- renderPlotly({
    ...
    items_problemas <- d$datos_con_pesos %>%
      ...
      group_by(VARIABLE) %>% summarise(Total_Fallas = n(), Incompletos = sum(CALIFICACION == 2),
                                       No_Existe = sum(CALIFICACION == 3), .groups = "drop") %>%
      arrange(desc(Total_Fallas)) %>% head(10) %>%
      mutate(ITEM_NUM = row_number(),
             VARIABLE_LIMPIA = iconv(as.character(VARIABLE), from = "UTF-8", to = "UTF-8", sub = ""),
             VARIABLE_CORTA = paste0(ITEM_NUM, ". ", truncar_texto(VARIABLE_LIMPIA, max_car)),
             VARIABLE_COMPLETA = VARIABLE_LIMPIA)
    
    if(nrow(items_problemas) == 0) return(plot_ly() %>% layout(title = "✅ Sin problemas", height = 300))
    
    fig <- plot_ly(items_problemas, x = ~Total_Fallas, y = ~VARIABLE_CORTA, type = 'bar', orientation = 'h', marker = list(color = '#e74c3c'),
                   hoverinfo = "text", hovertext = ~paste0("<b>", VARIABLE_COMPLETA, "</b><br>Fallas: ", Total_Fallas)) %>%
      layout(title = "Top 10 Problemas", xaxis = list(title = "Fallas"), yaxis = list(title = "Ítems", tickfont = list(size = 9), autorange = "reversed"),
             margin = list(l = 300, r = 50, t = 40, b = 50), height = 500)
    fig
  })
  ...
  output$tabla_completitud_problemas <- DT::renderDataTable({
    ...
    tryCatch({
      tabla_completitud <- d$datos_con_pesos %>%
        filter(SECCION == seccion_codigo, CALIFICACION != 4) %>%
        group_by(VARIABLE) %>% summarise(Total_Evaluados = n(), Completos = sum(CALIFICACION == 1),
                                         Incompletos = sum(CALIFICACION == 2), No_Existe = sum(CALIFICACION == 3),
                                         Porcentaje_Completitud = round((Completos / Total_Evaluados) * 100, 1),
                                         Total_Problemas = Incompletos + No_Existe, .groups = "drop") %>%
        mutate(ITEM = row_number(), Estado = case_when(Porcentaje_Completitud >= 90 ~ "Excelente",
                                                       Porcentaje_Completitud >= 70 ~ "Aceptable",
                                                       Porcentaje_Completitud >= 50 ~ "Deficiente", TRUE ~ "Inadecuado")) %>%
        select(ITEM, VARIABLE, Total_Evaluados, Completos, Incompletos, No_Existe, Porcentaje_Completitud, Total_Problemas, Estado) %>%
        arrange(desc(Porcentaje_Completitud))
      
      if(nrow(tabla_completitud) == 0) return(data.frame(Mensaje = "Sin datos"))
      
      DT::datatable(tabla_completitud, options = list(pageLength = 20, scrollX = TRUE, scrollY = "500px", autoWidth = TRUE),
                    rownames = FALSE, class = 'table table-striped table-bordered table-condensed', filter = 'top', escape = FALSE)
    }, error = function(e) {
      data.frame(Error = paste("Error:", e$message))
    })
  ...
  output$plot_top_criticos <- renderPlotly({
    req(datos_procesados())
    d <- datos_procesados()
    fig <- plot_ly(d$items_mas_criticos, x = ~TOTAL_FALLAS, y = ~VARIABLE, type = 'bar', orientation = 'h', marker = list(color = '#e74c3c'))
    fig %>% layout(title = "Items Críticos", yaxis = list(autorange = "reversed"), margin = list(l = 400, t = 40, b = 40))
  })
  ...
  output$plot_corr <- renderPlot({
    ...
    matriz_correlacion <- d$datos_con_pesos %>%
      group_by(ITEM, SECCION) %>% summarise(PCT = sum(PUNTAJE_OBTENIDO)/sum(PUNTAJE_MAXIMO)*100, .groups="drop") %>%
      pivot_wider(names_from = SECCION, values_from = PCT, values_fill = 0) %>% select(-ITEM) %>% cor()
    corrplot(matriz_correlacion, method = "circle", type = "upper", tl.col = "black", tl.srt = 45,
             col = colorRampPalette(c("#e74c3c", "white", "#2ecc71"))(200), title = "Correlación", mar = c(0,0,2,0))
  })
  
  output$tabla_historias_fallando <- DT::renderDataTable({
    req(datos_procesados())
    d <- datos_procesados()
    DT::datatable(d$historias_fallando, options = list(pageLength = 10, scrollX = TRUE, scrollY = "550px"),
                  class = 'table table-striped table-bordered table-condensed')
  })
  
  output$download_excel <- downloadHandler(
    filename = function() { paste0("Reporte_HC_", Sys.Date(), ".xlsx") },
    content = function(file) {
      req(datos_procesados())
      d <- datos_procesados()
      lista_export <- list(
        "01_RESUMEN" = as.data.frame(d$resumen_general),
        "02_SECCIONES" = as.data.frame(d$resumen_por_seccion),
        "03_SCORES" = as.data.frame(d$score_por_hc),
        "04_FALLANDO" = as.data.frame(d$historias_fallando),
        "05_CRITICOS" = as.data.frame(d$items_mas_criticos),
        "06_DATOS" = as.data.frame(d$datos_con_pesos))
      writexl::write_xlsx(lista_export, file)
    }
  )
  
  output$download_graficos_seccion <- downloadHandler(
    filename = function() { paste0("Graficos_", Sys.Date(), ".pdf") },
    content = function(file) {
      req(datos_procesados())
      d <- datos_procesados()
      pdf(file, width = 14, height = 10)
      for(seccion in unique(d$datos_con_pesos$SECCION)) {
        tratamiento_detalle <- d$datos_con_pesos %>% filter(SECCION == seccion) %>%
          group_by(VARIABLE, CALIFICACION_TXT) %>% summarise(Cantidad = n(), .groups = "drop")
        if(nrow(tratamiento_detalle) > 0) {
          p <- ggplot(tratamiento_detalle, aes(x = VARIABLE, y = Cantidad, fill = CALIFICACION_TXT)) +
            ...
            labs(title = paste(seccion), subtitle = paste("N =", d$resumen_general$TOTAL_HISTORIAS)) +
            scale_fill_manual(values = c("COMPLETO" = "#2ecc71", "INCOMPLETO" = "#f39c12", "NO EXISTE" = "#e74c3c", "NO APLICA" = "#95a5a6")) +
            theme_minimal() + theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 8), legend.position = "bottom")
          print(p)
        }
      }
      dev.off()

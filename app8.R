# ============================================================================
# AUDITORÍA DE CALIDAD DE HISTORIAS CLÍNICAS - VERSIÓN CORREGIDA Y MEJORADA
# ============================================================================
...
detectar_seccion <- function(nombre_archivo) {
  ...
  else if(grepl("ENFERMEDAD.*ACTUAL|ENFERMEDAD_ACTUAL", nombre_upper)) return("ENFERMEDAD_ACTUAL")
  else if(grepl("EXAMEN.*CLINICO|EXAMEN_CLINICO", nombre_upper)) return("EXAMEN_CLINICO")
  else if(grepl("DIAGNOSTICO", nombre_upper)) return("DIAGNOSTICOS")
  else if(grepl("PLAN.*TRABAJO|PLAN_TRABAJO", nombre_upper)) return("PLAN_TRABAJO")
  else if(grepl("TRATAMIENTO", nombre_upper)) return("TRATAMIENTO")
  else if(grepl("EVOLUCION|NOTAS.*EVOLUCION", nombre_upper)) return("NOTAS_EVOLUCION")
  else if(grepl("ENFERMERIA|REGISTROS.*ENFERMERIA", nombre_upper)) return("REGISTROS_ENFERMERIA")
...
# ============================================================================
# DATOS ESTÁTICOS: TABLA DE PESOS (COMPLETA)
# ============================================================================
...
# ============================================================================
# UI (INTERFAZ DE USUARIO) - MEJORADA
# ============================================================================
...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    ...
    box(
      ...
      collapsible = TRUE,
      collapsed = TRUE,
      HTML("Seleccione los archivos CSV de la auditoría para comenzar el análisis.")
    )
  ),
  dashboardBody(
    tags$head(
    ...
    tabItems(
      tabItem(tabName = "carga",
              fluidRow(
                box(width = 12, title = "📂 Cargar Archivos CSV", status = "primary", solidHeader = TRUE,
              ...
              fluidRow(
                box(width = 12, title = "📊 Estado de Carga", status = "warning",
                    uiOutput("mensaje_carga")
                )
              ),
              fluidRow(
                box(width = 12, title = "📋 Archivos Detectados", status = "info",
                    verbatimTextOutput("archivos_detectados")
                )
              )
      ),
...
# ============================================================================
# SERVER (LÓGICA MEJORADA)
# ============================================================================

server <- function(input, output, session) {
  
  # Reactive para procesar datos - VERSIÓN MEJORADA
  datos_procesados <- reactive({
    req(input$archivos_hc)
    
    archivos <- input$archivos_hc
    
    # Procesar cada archivo
    procesar_archivo <- function(ruta, nombre_archivo) {
      # Detectar sección mejorada
      ...
      df <- tryCatch({
        ...
        message("❌ Error leyendo ", nombre_archivo, ": ", e$message)
        NULL
      })
      
      if(is.null(df) || nrow(df) == 0) return(NULL)
      
      # Verificar que tenga columna ITEM
      if(!"ITEM" %in% names(df)) {
    ...
    lista_datos <- lapply(1:nrow(archivos), function(i) {
      procesar_archivo(archivos$datapath[i], archivos$name[i])
    })
    ...
    if(is.null(datos_crudos) || nrow(datos_crudos) == 0) {
      showNotification("❌ No se pudieron procesar los archivos. Verifique el formato.", type = "error")
      return(NULL)
    }
    ...
    # Calcular score por historia clínica - INCLUYENDO TODAS LAS SECCIONES
    ...
    # Resumen por sección - MEJORADO PARA INCLUIR TODAS
    ...
    resumen_por_seccion <- todas_secciones %>%
      ...
      mutate(
        PUNTAJE_OBTENIDO_SECCION = ifelse(is.na(PUNTAJE_OBTENIDO_SECCION), 0, PUNTAJE_OBTENIDO_SECCION),
        PUNTAJE_MAXIMO_SECCION = ifelse(is.na(PUNTAJE_MAXIMO_SECCION), 0, PUNTAJE_MAXIMO_SECCION),
    ...
    list(
      ...
      archivos_procesados = archivos$name
    )
  })
  ...
  output$mensaje_carga <- renderUI({
    ...
    tryCatch({
      ...
      tagList(
        ...
        p(strong("📁 Archivos procesados: "), length(d$archivos_procesados))
      )
    }, error = function(e) {
  ...
  output$archivos_detectados <- renderText({
    req(input$archivos_hc)
    paste("Archivos cargados:\n", paste(input$archivos_hc$name, collapse = "\n"))
  })
  ...
  # Barras por sección - MEJORADO PARA MOSTRAR TODAS
  ...
  # Gráfico de barras detallado por sección
  ...
  output$plot_completitud_items <- renderPlotly({
    ...
    fig <- plot_ly(completitud_items,
      ...
      layout(title = "Porcentaje de Completitud por Ítem",
             xaxis = list(title = "% Completitud", range = c(0, 100)),
  ...
  output$plot_items_problemas <- renderPlotly({
    ...
    fig <- plot_ly(items_problemas,
      ...
      layout(title = "Top 10 Ítems con Más Problemas",

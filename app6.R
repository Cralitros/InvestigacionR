...
truncar_texto <- function(texto, max_caracteres = 40) {
  # Convertir a character y manejar NA de forma vectorizada
  texto <- as.character(texto)
  texto[is.na(texto)] <- ""
  
  # Limpiar problemas de encoding (caracteres inválidos)
  texto <- iconv(texto, from = "UTF-8", to = "UTF-8", sub = "")
  
  # Truncar cada elemento del vector
...
server <- function(input, output, session) {
  ...
  output$plot_barras_seccion_detalle <- renderPlotly({
    ...
    tratamiento_detalle <- d$datos_con_pesos %>%
      ...
      mutate(
        ...
        # Limpieza de encoding antes de truncar
  ...
  # Reemplaza SOLO las funciones de tablas en el SERVER con estas:
  ...
  output$tabla_distribucion_items <- DT::renderDataTable({
    ...
    tryCatch({
      ...
      # Crear tabla con TODAS las combinaciones posibles
      ...
      # Unir y rellenar ceros donde no hay datos
      ...
      # Calcular totales (ahora seguro existen todas las columnas)
  ...
  output$download_tabla_distribucion <- downloadHandler(
    ...
    content = function(file) {
      ...
      tabla_datos <- d$datos_con_pesos %>%
        filter(SECCION == seccion_codigo) %>%
        group_by(VARIABLE, CALIFICACION_TXT) %>%
        summarise(Cantidad = n(), .groups = "drop") %>%
        tidyr::pivot_wider(names_from = CALIFICACION_TXT, 
                           values_from = Cantidad, 
  ...
  # TABLA 2: Completitud y problemas - CORREGIDO

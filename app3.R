# ============================================================================
# AUDITORÍA DE CALIDAD DE HISTORIAS CLÍNICAS - VERSIÓN FINAL PARA SHINYAPPS.IO
# CON DATOS PRE-CARGADOS Y UPLOAD OPCIONAL
# ============================================================================
...
# Opciones para manejar múltiples archivos
options(shiny.maxRequestSize = 50*1024^2)  # 50MB máximo
...
server <- function(input, output, session) {
  ...
  datos_procesados <- reactive({
    ...
    if(input$tipo_carga == "predefinido") {
      ...
      return(cargar_desde_upload_progresivo(input$archivos_hc))
    }
  })
  
  # CARGA PROGRESIVA DE ARCHIVOS
  cargar_desde_upload_progresivo <- function(archivos) {
    
    showNotification("📂 Iniciando carga de archivos...", type = "message", duration = 3)
    ...
    for(i in 1:nrow(archivos)) {
      tryCatch({
        ...
        showNotification(
          paste0("❌ Error en ", archivos$name[i], ": ", e$message),
    ...
    if(length(lista_datos) == 0) {
      showNotification("❌ Ningún archivo pudo ser procesado", type = "error", duration = 5)
      return(NULL)
    }
    ...
    showNotification(
      paste0("📊 Carga completada: ", archivos_procesados, " archivos, ",
             nrow(datos_crudos), " registros"),
...
cargar_datos_predefinidos <- function() {
  ...
  if(archivos_encontrados == 0) {
    showNotification("⚠️ No se encontraron archivos en la carpeta data/", type = "warning", duration = 5)
    return(NULL)
  }
  ...
  showNotification(
    paste0("✅ ", archivos_encontrados, " archivos pre-cargados, ",
           nrow(datos_crudos), " registros"),

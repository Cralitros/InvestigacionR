# ============================================================================
# AUDITORÍA DE CALIDAD DE HISTORIAS CLÍNICAS - APLICACIÓN SHINY COMPLETA
...
truncar_texto <- function(texto, max_caracteres = 40) {
  # Manejar NA y caracteres inválidos
  if(is.null(texto) || is.na(texto) || length(texto) == 0) {
    return("")
  }
  
  tryCatch({
    # Limpiar caracteres inválidos
    texto_limpio <- enc2utf8(as.character(texto))
    texto_limpio <- gsub("[^[:print:]]", "", texto_limpio)
    
    if(nchar(texto_limpio) > max_caracteres) {
      return(paste0(substr(texto_limpio, 1, max_caracteres), "..."))
    }
    return(texto_limpio)
  }, error = function(e) {
    # Si falla, devolver versión segura
    return(paste0("Item_", sample(1:1000, 1)))
  })
}
...
server <- function(input, output, session) {
  ...
  output$plot_barras_seccion_detalle <- renderPlotly({
    ...
    if(length(items_unicos) == 0) {
      return(plot_ly() %>% layout(title = "No hay datos para esta sección", height = 300))
    }
    ...
    tratamiento_detalle <- d$datos_con_pesos %>%
      ...
      mutate(
        ...
        VARIABLE_CORTA = sapply(VARIABLE, function(v) {
          texto <- truncar_texto(v, max_car)
          paste0(ITEM_NUM[which(VARIABLE == v)], ". ", texto)
        }),
  ...
  output$download_tabla_distribucion <- downloadHandler(
    filename = function() {
      paste0("Tabla_", gsub("[^a-zA-Z0-9]", "_", input$seccion_seleccionada), "_", Sys.Date(), ".csv")
    },
  ...
  # ✅ DESCARGA TABLA 1 - SOLO UNA FUNCIÓN (ELIMINADA LA DUPLICADA)
  output$download_tabla_distribucion <- downloadHandler(

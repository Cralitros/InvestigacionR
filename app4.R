# ============================================================================
# AUDITORÍA DE CALIDAD DE HISTORIAS CLÍNICAS - VERSIÓN FINAL
# ============================================================================
...
procesar_archivo_individual <- function(ruta, nombre_archivo) {
  seccion <- NA
...
cargar_datos_predefinidos <- function() {
  ...
  for(seccion in names(archivos)) {
    ruta <- archivos[[seccion]]
    if(file.exists(ruta)) {
      resultado <- procesar_archivo_individual(ruta, basename(ruta))
  ...
  if(archivos_encontrados == 0) return(NULL)
...
cargar_desde_upload <- function(archivos) {
  ...
  for(i in 1:nrow(archivos)) {
    resultado <- procesar_archivo_individual(archivos$datapath[i], archivos$name[i])
    if(!is.null(resultado) && nrow(resultado) > 0) {
      lista_datos[[i]] <- resultado
    }
    Sys.sleep(0.1)
  }
              ...
              fluidRow(
                box(width = 12, title = "📂 Opciones de Carga", status = "primary", solidHeader = TRUE,
                    radioButtons("tipo_carga", "Seleccione cómo cargar los datos:",
                                 ...
                                 selected = "predefinido", inline = TRUE)
                )
              ),
              fluidRow(
                box(width = 12, title = "📤 Subir Archivos CSV", status = "warning", solidHeader = TRUE,
                    conditionalPanel(
                      ...
                      fileInput("archivos_hc", "Seleccione los archivos CSV:",
                    ...
                    conditionalPanel(
                      ...
                      div(class = "alert alert-info",
                          icon("info-circle"), " Usando datos pre-cargados.")
                    )
                )
              ),
              fluidRow(
...
server <- function(input, output, session) {
  ...
  datos_procesados <- reactive({
    if(input$tipo_carga == "predefinido") {
      return(cargar_datos_predefinidos())
    } else {
  ...
  output$mensaje_carga <- renderUI({
    if(input$tipo_carga == "predefinido") {
      d <- datos_procesados()
      if(is.null(d)) {
        return(p("❌ No se encontraron archivos en data/"))
      }
      tagList(
        p("✅ Datos pre-cargados."),
        p(strong("📊 Total: "), d$resumen_general$TOTAL_HISTORIAS),
        p(strong("📈 Promedio: "), paste0(d$resumen_general$PROMEDIO_PUNTAJE, "%")))
    } else {
      if(is.null(input$archivos_hc)) return(p("⏳ Esperando..."))
      d <- datos_procesados()
      if(is.null(d)) return(p("❌ Error"))
      tagList(
        p("✅ Cargados."),
        p(strong("📊 Total: "), d$resumen_general$TOTAL_HISTORIAS),
        p(strong("📈 Promedio: "), paste0(d$resumen_general$PROMEDIO_PUNTAJE, "%")))
  ...
  output$plot_completitud_items <- renderPlotly({
    ...
    completitud_items <- d$datos_con_pesos %>%
      ...
      mutate(ITEM_NUM = row_number(), VARIABLE_CORTA = paste0(ITEM_NUM, ". ", truncar_texto(VARIABLE, max_car)), VARIABLE_COMPLETA = VARIABLE)
